...
/*获取高度值*/
function getHeight(){
	//用ID"copyFooter"标识含有版权信息div，高度为30，所以在计算tab高度时需要多减去30px
	if($("#copyFooter").length > 0){
		var footerHeight = $("#copyFooter").height();
		var rightHeight = document.body.clientHeight - 60 - footerHeight;
		   var Sys = {};
		  ...
		  try{
		      var s;
			     ...
			     if(Sys.ie) rightHeight = document.body.clientHeight - 60 - footerHeight;
			     if(Sys.firefox) rightHeight = document.body.clientHeight - 65 - footerHeight;
			     if(Sys.chrome) rightHeight = document.body.clientHeight - 60 - footerHeight;
			     if(Sys.opera) rightHeight = document.body.clientHeight - 207 - footerHeight;
			     if(Sys.safari) rightHeight = document.body.clientHeight - 65 - footerHeight;
			     //判断IE版本
			     if(Sys.ie && ua.indexOf('msie 8.0') > 1){
			     		rightHeight - 60 - footerHeight;
			     }
		  }catch(e){}
		  return rightHeight;
	}else{
	...
	$.ajax({
		...
		success : function(r){
			if(r){
				$.each(r,function(i,n){
					var child =$('<li></li>');
					var a=$('<a href="javascript:void(0);">'+n.lookupName+'</a>');
					(function(lookup){
						a.bind("click",function(e){
							e.preventDefault();
							if(lookup.lookupDes ==='js'){
								window[lookup.lookupCode]();
							}else if(lookup.lookupDes ==='menu'){
								$.ajax({
									url: 'platform/sysmenu/getSysmenuByCode/'+lookup.lookupCode+'.json',
									type :'get',
									async :false,
									dataType :'json',
									success :function(r){
...
function doLoadMenuScroll(){
	...
	if (tabsWidth > cWidth) {
		...
		if(false){
			...
			//ie6做特殊处理
			if(sRight.position().left > cWidth - sRight.outerWidth(true)){
				sRight.css('left', (cWidth - sRight.outerWidth(true)) + 'px');
				$('div.menu-arrow').css('left', (cWidth - sRight.outerWidth(true) - $('div.menu-arrow').outerWidth()) + 'px');
			}else{
			...
			//绑定事件
			var maxScrollWidth = tabsWidth - 1 - cWidth + sRight.outerWidth(true) + $('div.menu-arrow').outerWidth(true);
			$('div.menu-scroller-left').unbind().bind('click', function(e){
				var num = parseInt( $('div.menu-header').css('marginLeft'));
				var pos = Math.min(num + 200, sLeft.outerWidth(true));
				$("div.menu-header").animate({marginLeft: pos}, 400);
			})
