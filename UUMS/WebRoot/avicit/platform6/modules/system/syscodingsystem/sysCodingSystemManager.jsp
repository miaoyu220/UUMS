...
function doClick(node){
	...
	if(type == '1'){
		...
		$.ajax({
			url : 'platform/sysCodingSystemController/isHasChildren',
			data : {id : node.id},
			type : 'post',
			dataType : 'json',
...
function loadCriterionData(id){
	$.ajax({
		url : 'platform/sysCodingSystemController/getCriterionData',
		data : {id :id},
		type : 'post',
		dataType : 'json',
		async: false,
		success : function(result) {
			if(result && result.obj){
				...
				if(fileName == undefined || fileName == ""){
					...
					$('#documentFile').html(' ');
					$('#documentFile').html("<a href='platform/sysCodingSystemController/loadDocumentFile?id="+fileId+"' target='loadFileIframe'>"+fileName+"</a>");
				}
	...
	var buttons = [{
		...
		handler:function(){
			...
			$.ajax({
                cache: true,
                type: "POST",
                url:'platform/sysCodingSystemController/addCodingSystem',
                dataType:"json",
		...
		var buttons = [{
			...
			handler:function(){
				...
				$.ajax({
	                cache: true,
	                type: "POST",
	                url:'platform/sysCodingSystemController/updateCodingSystem',
	                dataType:"json",
	                data:frm.$('#formCodingSystem').serialize(),
	                async: false,
	                timeout:10000,
		...
		$.messager.confirm('确认', '您确认删除吗?', function(r){
            if (r){
        		...
        		$.ajax({
                    cache: true,
                    type: "POST",
                    url:'platform/sysCodingSystemController/deleteCodingSystem',
                    data:{id:id},
                    async: false,
                    timeout:10000,
                    dataType:"json",
...
function addCriterion(){
	...
	if (node){
		...
		$.ajax({
			url : 'platform/sysCodingSystemController/addCriterion',
			data : param,
			type : 'post',
			dataType : 'json',
			success : function(result) {
...
function editCriterion(){
	...
	$.ajax({
		url : 'platform/sysCodingSystemController/updateCriterion',
		data : param,
		type : 'post',
		dataType : 'json',
		success : function(result) {
...
function deleteCriterion(){
	$.messager.confirm('确认', '您确认删除吗?', function(r){
	  if (r){
		 ...
		 if(id != null && id != ""){
			$.ajax({
				url : 'platform/sysCodingSystemController/deleteCriterion',
				data : {id: id},
				type : 'post',
				dataType : 'json',
				success : function(result) {
...
<div region="west" border="true" title="编码体系维护" data-options="width: getTabWidth()" split="true">
	<div class="easyui-layout" fit="true">
		...
		<ul id="codingSystemTree" class="easyui-tree" data-options="
			lines : true,
			method : 'post',
			url:'platform/sysCodingSystemController/getCodingSystemTreeData.json',
					...
					<div class="formUnit column1">
						...
						<div class="inputContainer">
							<input style="width:400px" id="criterionLevel" name="criterionLevel" class="easyui-combobox"
								data-options="
									url:'platform/syslookuptype/getLookUpCode/PLATFORM_CODING_CRITERION_LEVEL.json',
									editable:false,
									method:'get',
									panelHeight:'auto',
