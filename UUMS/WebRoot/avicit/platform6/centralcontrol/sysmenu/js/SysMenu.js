...
SysMenu.prototype.loadMenuById=function(_self,node){
	$.ajax({
        cache: true,
        type: "get",
        url:_self.getUrl()+this._appId+"/sysmenu/"+node.id,
        dataType:"json",
        async: false,
        error: function(request) {
        	throw new Error('操作失败，服务请求状态：'+request.status+' '+request.statusText+' 请检查服务是否可用！');
...
SysMenu.prototype.insert=function(){
	...
	this._iDg = new CommonDialog("insert","700","400",this.getUrl()+this._appId+'/operation/add/'+this._selectNode._parentId,"添加菜单",false,true,false);
	this._iDg.show();
};
//添加子菜单
SysMenu.prototype.insertsub=function(){
	this._iDg=new CommonDialog("insert","700","400",this.getUrl()+this._appId+'/operation/add/'+this._selectNode.id,"添加菜单",false,true,false);
	this._iDg.show();
};
