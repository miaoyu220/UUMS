...
function openLayoutPage() {
	...
	var usd = new CommonDialog(dialogId, "450", "450",
			baseUrl + 'avicit/platform6/centralcontrol/sysportal/indexPortletConfig.jsp?appId='+appId+'&isgloable=true&sysPortletConfigId=<%=sysPortletConfigId%>&dialogId='+dialogId, "设置页面布局", false, true, false);
	usd.show();
}
function openPortletAdd() {
	var dialogId = "portletAddDialog";
	var usd = new CommonDialog(dialogId, "450", "450",
			baseUrl + 'avicit/platform6/centralcontrol/sysportal/indexPortletAdd.jsp?appId='+appId+'&isgloable=true&dialogId='+dialogId, "添加首页应用", false, true, false);
	usd.show();
}
function savePortlet(){
	...
	if(portlet!=null&&portlet!=""){
		...
		$.ajax({ 
			url: baseUrl+'platform/cc/sysportal/saveIndexPortlet.json',
			async : false,
			type: "POST",
...
<body class="easyui-layout"	onload="addTab('<%=PlatformLocalesJSTL.getBundleValue("portal.index.homePage")%>','avicit/platform6/centralcontrol/sysportal/indexPortlet.jsp?isgloable=true&sysPortletConfigId=<%=sysPortletConfigId%>','static/images/platform/index/images/icons.gif','homePage','-0px -20px');loadMessageData();mainOfQuickSearch();">
