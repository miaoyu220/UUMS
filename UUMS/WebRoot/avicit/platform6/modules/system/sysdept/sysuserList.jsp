 ...
 */
 function toLockLog(){
		...
		var path="platform/sysuser/toUserLockJsp?username="+rows[0].NO;
 ...
 * 从HR同步用户数据
 */
 function synchUserFromHr(){
	 $.messager.confirm('请确认','确认从权威数据源HR系统同步用户数据吗？',function(_flg){
		 if(_flg){
			 $.ajax({
			        //cache: true,
			        type: "POST",
			        url: "platform/uums/synch/fromhr/SynchUserFromHrController/synchSysUserFromHr",
			        dataType: "json",
			        data: '',
			        error: function(request) {
			        	$.messager.alert('提示','操作失败，服务请求状态：'+request.status+' '+request.statusText+' 请检查服务是否可用！','warning');
	...
	<div id="toolbarUser" >
						<table >
							...
							<sec:accesscontrollist hasPermission="3" domainObject="sysuser_toolbar_insertUser" permissionDes="添加">
								...
								<sec:accesscontrollist hasPermission="3" domainObject="sysuser_toolbar_sysnchOpen" permissionDes="同步用户">
								<td><a class="easyui-linkbutton" iconCls="icon-import" plain="true" onclick="synchUserFromHr();" href="javascript:void(0);">从HR同步用户</a></td>
								</sec:accesscontrollist>
