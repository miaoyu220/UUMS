 ...
 */
$(function(){
	Platform6.fn.lookup.getLookupType('PLATFORM_USER_LIMIT_IP_TYPE',function(r){
			if(r){
				ipLimitCombo=r;
				$('#filter_EQ_limitTypeIpType').combobox('loadData', [{lookupCode:'', lookupName:'所有'}].concat(ipLimitCombo));
			}
		});
	Platform6.fn.lookup.getLookupType('PLATFORM_USER_LIMTI_TYPE',function(r){
		if(r){
			userLimitCombo=r;
...
function formatIpLimit(value){
	return Platform6.fn.lookup.formatLookupType(value, ipLimitCombo);
};

function formatUserLimit(value){
	return Platform6.fn.lookup.formatLookupType(value, userLimitCombo);
};
