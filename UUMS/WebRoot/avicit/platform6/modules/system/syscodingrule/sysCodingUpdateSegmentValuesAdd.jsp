...
<div region="center" border="false">
   <form id="formSegmentValue" fit="true" style="padding-top: 5px;">
		...
		<table class="tableForm" width="100%" border="1" align="center">
			<c:if test="${segment.segmentRelation != null && segment.segmentRelation != ''}">
				...
				<c:forTokens var="num" items="${segment.segmentRelation}" delims="@@" varStatus="numStatus">
					<tr>
						...
						<td>
							<input id="segmentValue${numStatus.index}" segmentIndex="${num}" class="easyui-combobox" required="true" style="width: 205px"
								data-options="
			...
			<tr>
				...
				<td><input id="segmentName" name="segmentName" class="easyui-validatebox" required="true" data-options="validType:'length[1,100]'" style="width: 180px;"/></td>
			</tr>
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${segment.segmentLength gt 0}">
							<input id="segmentValue" name="segmentValue" class="easyui-validatebox" required="true" data-options="invalidMessage:'码段长度必须为${segment.segmentLength }',validType:'length[${segment.segmentLength },${segment.segmentLength }]'" style="width: 180px;"/>
						</c:when>
						<c:otherwise>
