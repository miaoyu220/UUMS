...
<div data-options="region:'center',split:true,border:false" style="overflow:hidden;padding-bottom:35px;">
					...
					<tr>
						<c:choose>
						   <c:when test="${item.colType== 'DATE'}">  
						   		<th>${item.colLabel}:</th> 
						   		<td width="80" align="right"><input name="${item.colName}" class="easyui-datetimebox" required="true" ></input></td>
						   </c:when>
						   <c:otherwise> 
						   				...
						   				<c:if test="${item.attribute01== null  and item.colRuleName== null and item.colName eq ptable.subColumnName}">
						   						<td width="80" align="right"><input id="${item.colName}"   name="${item.colName}" class="easyui-textbox" required="true"  type="hidden"></input></td>
						   				</c:if>
			...
			<table class="tableForm" border="0" cellspacing="1" width='100%'>
				<tr>	
					<td width="50%" align="right">
						<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="updateObj()" >保存</a>
						<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="javascript:parent.dlg_close('update')" >返回</a>
					</td>
				</tr>
			</table>
		</div>
	...
	<script type="text/javascript">
		...
		function updateObj() {
			url = 'platform/eform/formlayout/edit.json?id=' + '${id}'+'&tableId='+'${tableId}';
			$('#fmedit').form('submit', {
				url : url,
				onSubmit : function() {
