 ...
 * 从HR同步组织和部门数据
 */
 function synchOrgDepts(){
	 $.messager.confirm('请确认','确认从权威数据源HR系统同步组织部门数据吗？',function(_flg){
		 if(_flg){
			 $.ajax({
			        //cache: true,
			        type: "POST",
			        url:"platform/uums/synch/fromhr/SynchOrgFromHrController/synchOrgFromHr",
			        dataType:"json",
			        data:'',
			        error: function(request) {
			        	$.messager.alert('提示','操作失败，服务请求状态：'+request.status+' '+request.statusText+' 请检查服务是否可用！','warning');
...
<div data-options="region:'center',split:true,title:''" style="padding:0px;">
	...
	<div id="toolbarOrg" class="datagrid-toolbar">
			...
			<a id="btSynchOrgFromHr" class="easyui-linkbutton" iconCls="icon-add"  plain="true" onclick="synchOrgDepts();" href="javascript:void(0);">从HR同步组织部门</a>
	</div>
