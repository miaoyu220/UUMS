...
function CreateComponent(branchNo){
	...
	this.createTreeComponent = function(objectId,datas,treeUrl,beforeExpandUrl,tabNo,type){
		...
		object.tree({
			...
			onBeforeExpand:function(node,param){
				...
				if(type == 'dept'){
					object.tree('options').url = getPath() + beforeExpandUrl + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel;
				} else {
					object.tree('options').url = getPath() + beforeExpandUrl + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel;
				}
		    },
	...
	this.createSearchInputComponent = function(objectId,searchType,para){
		...
		$('#searcher #searcherInfo_' + objectId).liveSearch({url: getPath() + '/platform/user/bpmSelectUserAction/liveSearcher?searchType=' + searchType + '&para=' + para  + '&secretLevel=' + secretLevel + '&branchNo=' + branchNo + '&searchInfo='});
	};
}
/**
 * 绘制
