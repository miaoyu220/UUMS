		...
		if(_self.custom){
			buttons.push({
				text:'自定义',
	 			...
	 			handler:function(){
	 				try{
	 					window.showModalDialog(getPath() + "/platform/bpm/bpmdesigner/bpmWebDesigner/index?formCode=" + _self.formCode, window, "dialogHeight=2000px;dialogWidth:2000px;status=no;scroll=no");
	 					usd.close();
	 					_self.start();
	 				}catch(e){
	 					window.open(getPath() + "/platform/bpm/bpmdesigner/bpmWebDesigner/index?refreshFlg=1&formCode=" + _self.formCode, window, 'left=0,top=0,width='+ (screen.availWidth - 10) +',height='+ (screen.availHeight-50) +',scrollbars,resizable=yes,toolbar=no');
